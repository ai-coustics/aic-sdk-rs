...
fn generate_bindings() {
    ...
    let bindings = bindgen::Builder::default()
        ...
        // Force consistent behavior across platforms
        .default_enum_style(bindgen::EnumVariation::Consts)
        // Add type-safe constants that are consistently u32 across all platforms
        .raw_line("")
        .raw_line("// Cross-platform type-safe constants - always u32")
        .raw_line("// Use these instead of the raw constants for type consistency")
        .raw_line("pub const AIC_ERROR_CODE_SUCCESS: u32 = 0;")
        .raw_line("pub const AIC_ERROR_CODE_NULL_POINTER: u32 = 1;") 
        .raw_line("pub const AIC_ERROR_CODE_LICENSE_INVALID: u32 = 2;")
        .raw_line("pub const AIC_ERROR_CODE_LICENSE_EXPIRED: u32 = 3;")
        .raw_line("pub const AIC_ERROR_CODE_UNSUPPORTED_AUDIO_CONFIG: u32 = 4;")
        .raw_line("pub const AIC_ERROR_CODE_AUDIO_CONFIG_MISMATCH: u32 = 5;")
        .raw_line("pub const AIC_ERROR_CODE_NOT_INITIALIZED: u32 = 6;")
        .raw_line("pub const AIC_ERROR_CODE_PARAMETER_OUT_OF_RANGE: u32 = 7;")
        .raw_line("pub const AIC_ERROR_CODE_SDK_ACTIVATION_ERROR: u32 = 8;")
        .raw_line("")
        .raw_line("pub const AIC_MODEL_TYPE_QUAIL_L48: u32 = 0;")
        .raw_line("pub const AIC_MODEL_TYPE_QUAIL_L16: u32 = 1;")
        .raw_line("pub const AIC_MODEL_TYPE_QUAIL_L8: u32 = 2;")
        .raw_line("pub const AIC_MODEL_TYPE_QUAIL_S48: u32 = 3;")
        .raw_line("pub const AIC_MODEL_TYPE_QUAIL_S16: u32 = 4;")
        .raw_line("pub const AIC_MODEL_TYPE_QUAIL_S8: u32 = 5;")
        .raw_line("pub const AIC_MODEL_TYPE_QUAIL_XS: u32 = 6;")
        .raw_line("pub const AIC_MODEL_TYPE_QUAIL_XXS: u32 = 7;")
        .raw_line("")
        .raw_line("pub const AIC_PARAMETER_ENHANCEMENT_LEVEL: u32 = 0;")
        .raw_line("pub const AIC_PARAMETER_VOICE_GAIN: u32 = 1;")
        .raw_line("pub const AIC_PARAMETER_NOISE_GATE_ENABLE: u32 = 2;")
        .raw_line("")
        .raw_line("// Helper to cast function return values to u32 for consistency")
        .raw_line("pub fn to_u32_error_code(code: core::ffi::c_int) -> u32 { code as u32 }")
        .raw_line("pub fn from_u32_model_type(model_type: u32) -> core::ffi::c_int { model_type as core::ffi::c_int }")
        .raw_line("pub fn from_u32_parameter(parameter: u32) -> core::ffi::c_int { parameter as core::ffi::c_int }")
        .raw_line("")
        .raw_line("// Cross-platform function wrappers with consistent u32 types")
        .raw_line("pub unsafe fn aic_model_create_u32(model: *mut *mut AicModel, model_type: u32, license_key: *const core::ffi::c_char) -> u32 {")
        .raw_line("    to_u32_error_code(aic_model_create(model, from_u32_model_type(model_type), license_key))")
        .raw_line("}")
        .raw_line("pub unsafe fn aic_model_initialize_u32(model: *mut AicModel, sample_rate: u32, num_channels: u16, num_frames: usize) -> u32 {")
        .raw_line("    to_u32_error_code(aic_model_initialize(model, sample_rate, num_channels, num_frames))")
        .raw_line("}")
        .raw_line("pub unsafe fn aic_model_reset_u32(model: *mut AicModel) -> u32 {")
        .raw_line("    to_u32_error_code(aic_model_reset(model))")
        .raw_line("}")
        .raw_line("pub unsafe fn aic_model_process_planar_u32(model: *mut AicModel, audio: *const *mut f32, num_channels: u16, num_frames: usize) -> u32 {")
        .raw_line("    to_u32_error_code(aic_model_process_planar(model, audio, num_channels, num_frames))")
        .raw_line("}")
        .raw_line("pub unsafe fn aic_model_process_interleaved_u32(model: *mut AicModel, audio: *mut f32, num_channels: u16, num_frames: usize) -> u32 {")
        .raw_line("    to_u32_error_code(aic_model_process_interleaved(model, audio, num_channels, num_frames))")
        .raw_line("}")
        .raw_line("pub unsafe fn aic_model_set_parameter_u32(model: *mut AicModel, parameter: u32, value: f32) -> u32 {")
        .raw_line("    to_u32_error_code(aic_model_set_parameter(model, from_u32_parameter(parameter), value))")
        .raw_line("}")
        .raw_line("pub unsafe fn aic_model_get_parameter_u32(model: *const AicModel, parameter: u32, value: *mut f32) -> u32 {")
        .raw_line("    to_u32_error_code(aic_model_get_parameter(model, from_u32_parameter(parameter), value))")
        .raw_line("}")
        .raw_line("pub unsafe fn aic_get_output_delay_u32(model: *const AicModel, delay: *mut usize) -> u32 {")
        .raw_line("    to_u32_error_code(aic_get_output_delay(model, delay))")
        .raw_line("}")
        .raw_line("pub unsafe fn aic_get_optimal_sample_rate_u32(model: *const AicModel, sample_rate: *mut u32) -> u32 {")
        .raw_line("    to_u32_error_code(aic_get_optimal_sample_rate(model, sample_rate))")
        .raw_line("}")
        .raw_line("pub unsafe fn aic_get_optimal_num_frames_u32(model: *const AicModel, num_frames: *mut usize) -> u32 {")
        .raw_line("    to_u32_error_code(aic_get_optimal_num_frames(model, num_frames))")
        .raw_line("}")
        // Block the original enum constants to force use of our u32 versions
        .blocklist_var("AIC_ERROR_CODE_.*")
        .blocklist_var("AIC_MODEL_TYPE_.*") 
        .blocklist_var("AIC_PARAMETER_.*")
