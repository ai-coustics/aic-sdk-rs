...
fn handle_windows_linking(lib_path: &Path) {
    ...
    // Check DLL dependencies for debugging
    println!("cargo:warning=aic.dll dependencies include VCRUNTIME140.dll (Visual C++ Runtime)");
    println!("cargo:warning=If you get STATUS_DLL_NOT_FOUND, install Visual C++ Redistributable x64");
    
    // Always generate our own import library to avoid symbol conflicts
    if try_generate_import_library(&dll_path, &out_dir) {
        println!("cargo:rustc-link-search=native={}", out_dir.display());
        println!("cargo:rustc-link-lib=dylib=aic");
    } else {
        println!("cargo:warning=Couldn't generate import library, trying direct DLL linking");
        println!("cargo:rustc-link-search=native={}", dll_path.parent().unwrap().display());
        println!("cargo:rustc-link-lib=dylib=aic");
    }
    
    // Copy DLL to target directories
    copy_dll_to_multiple_locations(&dll_path);
}
...
fn try_generate_import_library(dll_path: &Path, out_dir: &Path) -> bool {
    ...
    // Create a .def file with actual function names from the AIC SDK
    let def_content = r#"EXPORTS
    aic_model_create
    aic_model_destroy
    aic_model_initialize
...
"#;
    ...
    if let Err(_) = std::fs::write(&def_file_path, def_content) {
        return false;
    }
    
    // Try dlltool (MinGW), lib.exe (MSVC), or llvm-dlltool
    let def_path_str = def_file_path.to_string_lossy();
    let lib_path_str = import_lib_path.to_string_lossy();
    let def_arg = format!("/DEF:{}", def_file_path.display());
    let out_arg = format!("/OUT:{}", import_lib_path.display());
    
    let tools = [
        ("dlltool", vec!["-d", def_path_str.as_ref(), "-l", lib_path_str.as_ref(), "-m", "i386:x86-64"]),
        ("lib", vec![def_arg.as_str(), out_arg.as_str(), "/MACHINE:X64"]),
        ("llvm-dlltool", vec!["-d", def_path_str.as_ref(), "-l", lib_path_str.as_ref(), "-m", "i386:x86-64"]),
    ];
    
    for (tool, args) in &tools {
        if let Ok(output) = Command::new(tool).args(args).output() {
            if output.status.success() {
                println!("cargo:warning=Generated import library using {}", tool);
                let _ = std::fs::remove_file(&def_file_path);
                return true;
            }
        }
...
fn copy_dll_to_multiple_locations(dll_path: &Path) {
    let profile = env::var("PROFILE").unwrap_or_else(|_| "debug".to_string());
    let manifest_dir = PathBuf::from(env::var("CARGO_MANIFEST_DIR").unwrap());
    ...
    if let Some(workspace_root) = manifest_dir.parent() {
        ...
        // Copy to main target directory
        let main_dll = target_dir.join("aic.dll");
        if let Ok(_) = std::fs::copy(dll_path, &main_dll) {
            println!("cargo:warning=Copied DLL to: {}", main_dll.display());
        }
        
        // Copy to examples directory
        let examples_dir = target_dir.join("examples");
        let _ = std::fs::create_dir_all(&examples_dir);
        let examples_dll = examples_dir.join("aic.dll");
        if let Ok(_) = std::fs::copy(dll_path, &examples_dll) {
            println!("cargo:warning=Copied DLL to examples: {}", examples_dll.display());
        }
    }
}
