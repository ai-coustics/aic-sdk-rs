...
pub fn patch_lib(
    ...
    println!(
        "cargo:warning=No suitable tools found for symbol filtering on Windows, using library as-is"
    );
    fs::copy(static_lib, final_lib).expect("Failed to copy library file");
}

fn try_llvm_approach(
    static_lib: &Path,
    out_dir: &Path,
    _lib_name: &str,
    lib_name_patched: &str,
    ...
    if !is_command_available("llvm-ar")
        || !is_command_available("llvm-objcopy")
        || !is_command_available("llvm-nm")
    {
        return false;
    }
    ...
    let extract_success = match extract_output {
        ...
        Ok(output) => {
            println!(
                "cargo:warning=llvm-ar extraction failed: {}",
    ...
    for entry in entries {
        ...
        if should_keep {
            ...
            let filtered_obj = work_dir.join(format!(
                "filtered_{}",
    ...
    let ar_success = match cmd.output() {
        ...
        Ok(output) => {
            println!(
                "cargo:warning=llvm-ar library creation failed: {}",
                String::from_utf8_lossy(&output.stderr)
            );
            false
        }
        Err(e) => {
            println!(
                "cargo:warning=Failed to execute llvm-ar for library creation: {}",
                e
            );
            false
        }
...
fn try_msvc_approach(
    ...
    let list_output = Command::new("lib").arg("/LIST").arg(static_lib).output();
    ...
    for obj_name in &object_names {
        ...
        if let Ok(status) = extract_status {
            ...
            println!(
                "cargo:warning=Failed to execute lib /EXTRACT for {}",
                obj_name
            );
            extract_success = false;
            break;
...
fn create_filtered_library_msvc(
